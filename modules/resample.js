...
function smoothHeightmap() {
    grid.cells.h.forEach((height, newGridCell) => {
      ...
      grid.cells.h[newGridCell] = isWaterCell(grid, newGridCell) ? Math.min(meanHeight, 19) : Math.max(meanHeight, 20);
    });
}
...
function restoreCellData(parentMap, inverse, scale) {
    ...
    for (const newPackCell of pack.cells.i) {
      ...
      if (isWaterCell(pack, newPackCell)) continue;
...
function restoreBurgs(parentMap, projection, scale) {
    ...
    pack.burgs = parentMap.pack.burgs.map(burg => {
      ...
      const cell = isWaterCell(pack, closestCell) ? findLandCell(xp, yp) : closestCell;
...
function groupCellsByType(graph) {
    return graph.cells.p.reduce(
      (acc, [x, y], cellId) => {
        const group = isWaterCell(graph, cellId) ? "water" : "land";
...
function isWaterCell(graph, cellId) {
